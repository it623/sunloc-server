...
app.get('/api/tracking/labels-all', async (req, res) => {
  try {
    ...
    // v41h PERF FIX: generated column stores ISO timestamps (e.g. 2026-03-21T09:06:45.261Z)
    // Filter to last 60 days using ISO string comparison to reduce payload from ~10k to ~5k labels
    const sinceParam = req.query.since || null;
    const sinceDate = sinceParam || (() => {
      const d = new Date(); d.setDate(d.getDate() - 60);
      return d.toISOString().slice(0, 10); // YYYY-MM-DD
    })();
    // ISO string comparison works because '2026-04-01' < '2026-04-01T09:...' lexicographically
