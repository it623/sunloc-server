...
app.get('/api/planning/state', async (req, res) => {
  try {
    ...
    // v41ZI Item 4+6: ensure the actuals/gross/override caches are warm so the gross we inject
    // below is current on this very GET (throttled to 60s internally, so this is cheap). Without it
    // a cold planning/state read would skip injection until a later warm cycle.
...
app.get('/api/tracking/scan-summary', async (req, res) => {
  try {
    ...
    // Reports D & E show the DPR-entered gross for EVERY batch instead of relying on the planning
    // order's injected actualProd (which could be blank when the actuals cache hadn't keyed it).
    await warmActualsCache().catch(()=>{});
    const grossByBatch = {};
