...
async function _doRefreshSapInvoices() {
  ...
  if (!r.ok) return { ok: false, error: r.error, degraded: r.degraded, fetched: 0, upserted: 0, serverBuild: 'v45ZU' };
  ...
  return { ok: true, fetched: invoices.length, upserted, serverBuild: 'v45ZU' };
}
...
app.get('/api/health', (req, res) => {
  try {
    ...
    res.json({ ok: true, server: 'Sunloc Integrated Server v1.0', build: 'v45ZU', db: DB_PATH, uptime: Math.floor(process.uptime())+'s', note: 'DB initialising: '+err.message });
  }
});
