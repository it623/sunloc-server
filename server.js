...
app.get('/api/tracking/scan-summary', async (req, res) => {
  try {
    ...
    if (pgPool) {
      // v37G FIX: previously used `tracking_dispatch_recs` (wrong name — table is
      // `tracking_dispatch_records`). Promise.all rejection on the bad query was
      // returning HTTP 500 and breaking every report. Each query is now individually
      // guarded so one bad table never takes down the whole endpoint.
      ...
      // v37G FIX: corrected table name (was tracking_dispatch_recs)
    ...
    // Build per-batch summary: { [batchNo]: { aim:{in,out,inQty,outQty}, printing:{...}, pi:{...}, packing:{...}, dispatch:{...} } }
    const summary = {};
...
app.get('/api/tracking/agrade-summary', async (req, res) => {
  try {
    ...
    Object.entries(batches).forEach(([batchNo, depts]) => {
      ...
      // WIP = Gross Production − All Wastage and Remelt across all stages − Pack-In Quantity
      // "Pack-In Quantity" = boxes that have ARRIVED at the packing department (cleared production).
      // Once material reaches packIn, it has cleared all production stages and is Finished Goods.
      // FG awaiting dispatch (packIn − packOut) is NOT WIP — it's queued for next available vehicle,
      // handled by dispatch scheduling, not by WIP-days projection.
      // WIP Days = wipLakhs / machine cap projects when *future* material reaches FG.
      // FG already at packing (waiting to ship) is independent of this calculation.
      const totalWastageForWIP = aimWaste + printWaste + piWaste;
      const wipLakhs = Math.max(0, grossProd - totalWastageForWIP - packInQty);
      const fgAwaitingDispatch = Math.max(0, packInQty - packOutQty); // FG ready, awaiting vehicle
      ...
      result[batchNo.toUpperCase()] = { // normalize to uppercase for consistent lookup
        ...
        grossProd,
        wipLakhs,         // Lakhs still in production (not yet reached packing) — drives WIP Days
        fgAwaitingDispatch  // Lakhs at packing dept waiting for vehicle — handled by dispatch scheduling
      };
    });
